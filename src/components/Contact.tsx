          ...
          <div className="mt-10 flex flex-wrap gap-3">
            {links.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="rounded-full border border-foreground px-5 py-3 text-xs font-bold uppercase tracking-[0.18em] hover:border-accent hover:bg-accent hover:text-background"
              >
                {link.label}
              </a>
            ))}
